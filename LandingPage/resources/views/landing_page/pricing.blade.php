        ...
        <div class="container-v5">
            ...
            <div class="card-grid" style="--card-min: 300px; --card-max: 440px; --card-gap: 2rem; margin-bottom: 4rem;">
                ...
                <div style="background: {{ $plan['highlighted'] ? 'var(--contrast-panel-bg)' : 'var(--surface)' }}; border-radius: var(--card-radius); padding: 2.5rem; border: {{ $plan['highlighted'] ? 'none' : '1px solid var(--border)' }}; position: relative; transition: all 0.3s ease;"
                    ...
                    <a href="{{ route('landing.contact') }}" style="background: {{ $plan['highlighted'] ? 'var(--contrast-action-bg)' : 'var(--button-primary-bg)' }}; color: {{ $plan['highlighted'] ? 'var(--contrast-action-text)' : 'var(--button-primary-text)' }}; padding: 0.75rem 1.5rem; border-radius: 8px; font-weight: 600; text-decoration: none; display: block; text-align: center; transition: all 0.3s ease;"
